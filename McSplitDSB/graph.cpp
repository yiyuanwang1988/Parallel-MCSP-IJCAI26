...
dsbGraph::Graph::Graph(unsigned int n) {
    this->n = n;
    label = std::vector<unsigned int>(n, 0u);
    adjmat = {n, std::vector<unsigned int>(n, false)};
}

dsbGraph::Graph dsbGraph::induced_subgraph(struct dsbGraph::Graph& g, std::vector<int> vv) {
    dsbGraph::Graph subg(vv.size());
...
void add_edge(dsbGraph::Graph& g, int v, int w, bool directed=false, unsigned int val=1) {
    if (v != w) {
        if (directed) {
            g.adjmat[v][w] |= val;
            g.adjmat[w][v] |= (val<<16);
...
static struct dsbGraph::Graph readDimacsGraph(char* filename, bool directed, bool vertex_labelled) {
    struct dsbGraph::Graph g(0);
...
static struct dsbGraph::Graph readLadGraph(char* filename, bool directed) {
    struct dsbGraph::Graph g(0);
    FILE* f;
...
static struct dsbGraph::Graph readBinaryGraph(char* filename, bool directed, bool edge_labelled,
...
struct dsbGraph::Graph dsbGraph::readGraph(char* filename, char format, bool directed, bool edge_labelled, bool vertex_labelled) {
    struct dsbGraph::Graph g(0);
